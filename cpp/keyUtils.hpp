...
static void KangarooTwelve_F_Absorb(KangarooTwelve_F *instance, const uint8_t *data, unsigned long long dataByteLen)
{
    ...
    while (i < dataByteLen)
    {
        if (!instance->byteIOIndex && dataByteLen >= i + K12_rateInBytes)
        {
            ...
            copyFromState(stateAsLanes) unsigned long long modifiedDataByteLen = dataByteLen - i;
...
{ // 256-bit Montgomery multiplication modulo the curve order, mc = ma*mb*r' mod order, where ma,mb,mc in [0, order-1]
    ...
    if (_addcarry_u64(_addcarry_u64(_addcarry_u64(_addcarry_u64(_addcarry_u64(_addcarry_u64(_addcarry_u64(_addcarry_u64(0, P[0], temp[0], &temp[0]), P[1], temp[1], &temp[1]), P[2], temp[2], &temp[2]), P[3], temp[3], &temp[3]), P[4], temp[4], &temp[4]), P[5], temp[5], &temp[5]), P[6], temp[6], &temp[6]), P[7], temp[7], &temp[7]) - _subborrow_u64(_subborrow_u64(_subborrow_u64(_subborrow_u64(0, temp[4], CURVE_ORDER_0, &mc[0]), temp[5], CURVE_ORDER_1, &mc[1]), temp[6], CURVE_ORDER_2, &mc[2]), temp[7], CURVE_ORDER_3, &mc[3]))
    {
...
{ // Point validation: check if point lies on the curve
    ...
    return ((!(t1[0][0] | t1[0][1]) || !((t1[0][0] + 1) | (t1[0][1] + 1))) && (!(t1[1][0] | t1[1][1]) || !((t1[1][0] + 1) | (t1[1][1] + 1))));
}
...
{ // Decode point P
    ...
    mod1271(P->x[0]);
    if (((unsigned int)(Pencoded[31] >> 7)) != (unsigned int)(P->x[(!P->x[0][0] && !P->x[0][1]) ? 1 : 0][1] >> 62)) // If sign of x-coordinate decoded != input sign bit, then negate x-coordinate
    {
        fp2neg1271(P->x);
    }
...
static void getPublicKeyFromSeed(const char *seed, uint8_t *publicKey)
{
    ...
    std::cout << "trying to get public key from private key" << std::endl;
...
{ // Secret agreement computation for key exchange using a compressed, 32-byte public key
  ...
  // Output: 32-byte sharedKey
    ...
    if (!A->x[0][0] && !A->x[0][1] && !A->x[1][0] && !A->x[1][1] && A->y[0][0] == 1 && !A->y[0][1] && !A->y[1][0] && !A->y[1][1]) // Is output = neutral point (0,1)?
    {
        return false;
    }
