...
#include "mining/score_engine.hpp"
...
struct ScoreFunction
{
    ...
    static bool isValidScore(unsigned int solutionScore, score_engine::AlgoType selectedAlgo)
    {
        if (selectedAlgo == score_engine::AlgoType::HyperIdentity)
        {
            return (solutionScore >= 0) && (solutionScore <= HYPERIDENTITY_NUMBER_OF_OUTPUT_NEURONS) && (solutionScore != score_engine::INVALID_SCORE_VALUE);
        }
        else if (selectedAlgo == score_engine::AlgoType::Addition)
        {
            return (solutionScore >= 0) && (solutionScore <= ADDITION_NUMBER_OF_OUTPUT_NEURONS * (1ULL << ADDITION_NUMBER_OF_INPUT_NEURONS)) && (solutionScore != score_engine::INVALID_SCORE_VALUE);
        }
        return false;
    }
    static bool isGoodScore(unsigned int solutionScore, int threshold, score_engine::AlgoType selectedAlgo)
    {
