...
namespace NodeManager {
  ...
  export async function loadFromDisk() {
    ...
    try {
      let diskSolutionsToSubmitQueue = JSON.parse(
        fs.readFileSync(`${DATA_PATH}/solutionsToSubmitQueue.json`, "utf-8")
      );
  ...
  export function checkAndRemoveIpsIfInactive(
    ...
    for (let i = 0; i < nodeIps.length; i++) {
      ...
      if (
        ...
        (ip === checkTickIp.ip &&
          checkTickIp.tick > 0 &&
          checkTickIp.tick < Explorer.ticksData.tickInfo.tick - MAX_TICK_BEHIND)
      ) {
        deletedIps.push(ip);
      ...
      addon.pay(
        ...
        (tick, txhash) => {
          if (tick > 0) {
            ...
            reject(new Error(`transaction failed by node connection`));
          }
        }
      );
    });
  ...
  export function watchAndSubmitSolution() {
    ...
    setInterval(async () => {
      ...
      try {
        ...
        isProcessing = false;
        LOG("error", "NodeManager.watchAndSubmitSolution: " + e.message);
      }
    }, ONE_SECOND / 2);
  }
  ...
  export async function init(ips: string, secretSeed: string) {
    ...
    let rawSolutionData = prepareSolutionData(
      ...
      "SINUBYSBZKBSVEFQDZBQWUEJWRXCXOZNKPHIXDZWRBKXDSPJEHFAMBACXHUN"
    );
    ...
    try {
      ...
      LOG(
        "error",
        "NodeManager.init: failed to send test solution: " + error.message
      );
    }
  ...
  export function prepareSolutionData(
    ...
    if (!myIndentity || myIndentity.length !== 60) {
      throw new Error("Identity is not set, please init node manager first");
    }
  ...
  export async function sendSolutionV2(data: Uint8Array): Promise<boolean> {
    return new Promise(async (resolve, reject) => {
      ...
      addon.sendSolutionV2("146.0.75.252", data, (isOK: boolean) => {
        if (isOK) {
          resolve(isOK);
        } else {
